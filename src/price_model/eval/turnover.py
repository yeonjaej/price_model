...
2. Define a weight vector `w_t` over the union of tickers: `+1/k` if in
   ...
   date `t`.
3. One-sided daily turnover:
       turnover_t = 0.5 * sum_i | w_t[i] - w_{t-1}[i] |
   A turnover of 1.0 corresponds to fully replacing every position.
4. Long-short gross return: `mean(top.realized) - mean(bot.realized)` (the
   same definition used by the existing metrics module).
5. After-cost daily return at cost `b` basis points per side:
       net_ret_t = gross_ret_t - turnover_t * b / 10000
6. Annualize both gross and net Sharpe via `sqrt(252 / horizon_days)`,
   matching the existing `summarize` convention.

Turnover is reported as both the daily mean and the annualized fraction
(`daily_mean * 252`). Annualized turnover of 1.0 means the portfolio
replaces itself once per year on average; institutional momentum books
typically run 1.0 to 3.0 annual turnover, daily-rebalanced ML signals on
liquid universes often run 5.0 to 15.0.
...
This module is deliberately conservative: cost is applied symmetrically to
the long and short legs, so the cost line item `turnover * bps` charges
the full two-sided rebalance.
"""
...
class CostAdjustedSummary:
    ...
    # Turnover (one-sided)
    daily_turnover_mean: float  # average daily fraction, e.g., 0.04 = 4%/day
    annual_turnover: float  # daily_mean * 252, e.g., 10.0 = 1000%/yr
...
def _one_sided_turnover_series(weights_seq: list[dict[str, float]]) -> list[float]:
    """For each date t, turnover_t = 0.5 * sum_i | w_t[i] - w_{t-1}[i] |.

    First date's turnover is the cost of building the portfolio from scratch
    (sum of |w|/2 = 1.0 for a balanced long-short). This is realistic
    because day-1 incurs the full setup trade.
    """
...
def compute_turnover_and_costs(
    ...
    *,
    ...
    turnover_seq = _one_sided_turnover_series(weights_seq)
    ...
    daily_to = float(turnover_arr.mean()) if turnover_arr.size > 0 else float("nan")
    annual_to = daily_to * 252 if not math.isnan(daily_to) else float("nan")

    # After-cost Sharpe per cost level
    after_cost: dict[int, float] = {}
    for bp in cost_bps:
        bp_int = int(bp)
        cost_decimal = bp_int / 10000.0
        net = ret_arr - turnover_arr * cost_decimal
