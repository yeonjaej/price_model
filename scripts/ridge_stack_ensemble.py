"""Ridge-stacked ensemble of cross-sectional return predictors.

Replaces the hand-coded softmax blend (`scripts/regime_conditional_ensemble.py`,
`scripts/walk_forward_ensemble.py`) with a Ridge regression that learns
optimal linear weights on the base-model predictions from data, rather than
from a fixed regime-bucket structure.

The key insight: each base model produces out-of-fold predictions on every
(date, ticker) in the apples-to-apples slice (the walk-forward harness
already does this — its predictions ARE out-of-fold from the underlying
base learner's perspective). Stacking these predictions with a Ridge meta-
model on a held-out training half learns the optimal linear combination
without requiring any regime conditioning. This is the standard Kaggle-
style winning move for cross-sectional return prediction.

Lookahead-safety
----------------
1. The base-model predictions are already out-of-fold (produced by walk-
   forward harness with embargo).
2. The training-half / eval-half split is FIXED at the midpoint of the
   apples-to-apples slice.
3. The Ridge meta-model is fit on the train half ONLY; the alpha
   selection is via cross-validation INSIDE the train half.
4. The trained Ridge is applied verbatim to the eval half — no eval-half
   information enters the weight selection.

This is a strict OOS evaluation of the stacking layer.

Why Ridge (not Lasso, not OLS, not stacking-via-LightGBM):
- Ridge handles collinearity smoothly (base-model predictions are highly
  correlated by construction — they all predict the same target).
- Lasso would zero some base models and overfit to the strongest one on
  the train half — exactly the failure mode the static softmax ensemble
  exhibited.
- OLS would be unstable on correlated regressors.
- LightGBM-as-meta would introduce another overfit layer.

Ridge is the textbook stacking meta-model in the asset-pricing ML
literature (Gu-Kelly-Xiu 2020, Han-He-Rapach-Zhou 2024).

Usage:
    PYTHONPATH=src python scripts/ridge_stack_ensemble.py
    PYTHONPATH=src python scripts/ridge_stack_ensemble.py --regime-conditional
        # ↑ fits a separate Ridge per dispersion bucket (3 buckets by default)
"""
...
def _fit_ridge_and_apply(
    ...
    """Fit RidgeCV on (train_X, train_y), apply to eval_X. Returns
    (eval_predictions, diagnostic_dict)."""
    from sklearn.linear_model import RidgeCV

    if alpha_grid is None:
        alpha_grid = np.logspace(-3, 5, 40).tolist()
    ridge = RidgeCV(alphas=alpha_grid, cv=cv, fit_intercept=True)
    ridge.fit(train_X, train_y)
    eval_pred = ridge.predict(eval_X)
    diagnostics = {
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        help="Fit a separate Ridge per dispersion bucket instead of one global Ridge.",
    )
    ap.add_argument(
    ...
    ap.add_argument(
        ...
        help="K-fold cross-validation folds for RidgeCV alpha selection.",
    )
    ap.add_argument(
    ...
    # ----- 3. Realized + (optional) dispersion -----
    ...
    # Drop rows where any base model's prediction is null OR realized target
    # is null (latter happens near the end of the panel where the 5-day
    # forward return isn't yet observable).
    ...
    # ----- 7. Fit Ridge stacking -----
    if not args.regime_conditional:
        # ----- 7a. Global Ridge stack (one model on all train data) -----
        ...
        eval_pred, diag = _fit_ridge_and_apply(train_X, train_y, eval_X, cv=args.cv)
        console.rule("[bold]Ridge stacking (global)")
        table = Table()
        table.add_column("base model")
        table.add_column("weight (Ridge coefficient)", justify="right")
        for m, c in zip(target_ids, diag["coefficients"], strict=True):
            table.add_row(m, f"{c:+.4f}")
        table.add_row("intercept", f"{diag['intercept']:+.6f}")
        table.add_row("alpha (CV-selected)", f"{diag['alpha']:.4g}")
        console.print(table)
        ...
        # ----- 7b. Regime-conditional Ridge (one model per bucket) -----
        ...
        console.rule("[bold]Ridge stacking (per regime bucket)")
        table = Table()
        ...
        for b in range(args.n_buckets):
            ...
            if train_b.height < 100:
                console.print(
                    ...
                    f"falling back to global ridge weights.[/yellow]"
                )
                # Use global ridge as fallback
            ...
            if test_X_b.shape[0] == 0:
                continue
            pred_b, diag_b = _fit_ridge_and_apply(
                train_X_b, train_y_b, test_X_b, cv=args.cv
            )
            # Map back to original test_pivot ordering using ticker+date
            # Build an index alignment
            ...
            cells.append(f"{diag_b['alpha']:.4g}")
            table.add_row(*cells)
        console.print(table)
        test_pivot = test_pivot.with_columns(
            pl.Series("ensemble_prediction", eval_pred)
    ...
    label = "ENSEMBLE_RIDGE_REGIME" if args.regime_conditional else "ENSEMBLE_RIDGE"
